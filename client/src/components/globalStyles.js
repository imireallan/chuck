...
export const Button = styled.button`
	...
	background: ${(props) => (props.large ? 'var(--color-orange)' : 'transparent')};
	border: 0;
	outline: 0;
	...
	&:focus {
		color: ${(props) => (props.large ? '#333' : 'var(--color-white)')};
		background: ${(props) => (props.large ? 'transparent' : 'var(--color-orange)')};
		border: ${(props) => (props.large ? '2px solid var(--color-orange)' : 'none')};
	}
`;
